...
export type PluginUploaderApi = UploaderApi;
...
/**
 * v1-shape plugin definition. Kept for internal v1 plugins
 * (`cloudImageEditorPlugin`, `instagramPlugin`) and the
 * `plugin-api-bridge` that routes them through v2 controllers.
 * Renamed from `UploaderPlugin` because the public export now points
 * at the v2 `PluginDefinition` under that name (see
 * `src/types/exported.ts`).
 */
export type LegacyUploaderPlugin = {
  id: string;
