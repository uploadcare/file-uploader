 ...
 *
 * A `declare`-only token: the single member is type-level (no runtime body), so
 * the class compiles to ~0 bytes — it exists purely as the container token + the
 * structural type the bound factory's object literal satisfies. It is never
 * `new`-ed; the container resolves it through the factory `ensurePluginManager`
 * binds (`() => container.get(PluginController)`). A concrete (non-abstract)
 * class so it stays assignable as a `Token<T>`.
 *
