...
import { localeStateKey } from './managers/LocaleManager';

export function l10nProcessor<T extends Block>(fr: DocumentFragment, fnCtx: T): void {
  [...fr.querySelectorAll('[l10n]')].forEach((el: Element) => {
    const key = el.getAttribute('l10n');
    if (!key) {
      return;
    }
    const list: string[] = key.split(';');

    for (const item of list) {
      if (item) locale(el, item, fnCtx as any);
    }
  });
}
...
const locale = (el: Element, key: string, fnCtx: any): void => {
  ...
  if (key.includes(':')) {
    ...
    const tuple = arr as [string, string];
    elProp = tuple[0];
    key = tuple[1];
  ...
  if (fnCtx.has(localCtxKey)) {
    fnCtx.sub(localCtxKey, (mappedKey: string) => {
      if (!mappedKey) {
        return;
      }
      ...
      keySubs?.forEach((sub: { remove: () => void }) => {
        sub.remove();
        keySubs.delete(sub);
      ...
      const sub = fnCtx.nodeCtx.sub(nodeStateKey, () => {
        (el as any)[elProp as 'textContent'] = fnCtx.l10n(mappedKey);
      });
      keySubs?.add(sub);
  ...
  fnCtx.sub(stateKey, () => {
    if (useAttribute) {
      ...
      (el as any)[elProp as 'textContent'] = fnCtx.l10n(key);
    }
  });
  el.removeAttribute('l10n');
};
