/**
 * Helper function for legacy browsers and iframes which sometimes focus elements like document, body, and non-interactive SVG.
 *
...
/**
 * Applies the :focus-visible polyfill at the given scope. A scope in this case is either the top-level Document or a Shadow Root.
 *
...
export function applyFocusVisiblePolyfill(scope, callback) {
   ...
   * avoids the situation where a user presses a key on an already focused element, and then clicks on a different
  ...
  /**
   * When the polfyill first loads, assume the user is in keyboard modality. If any event is received from a pointing
