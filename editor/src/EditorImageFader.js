...
import { preloadImage, batchPreloadImages } from './lib/preloadImage.js';
...
export class EditorImageFader extends AppComponent {
  constructor() {
    ...
    applyElementStyles(this, STYLES['inactive_to_cropper']);
  }
  ...
  _addKeypoint(operation, filter, value) {
    ...
    applyElementStyles(image, STYLES.image);
  ...
  _createPreviewImage() {
    let image = new Image();
    applyElementStyles(image, STYLES.image);
    applyElementStyles(image, STYLES['image--preview']);
  ...
  async _initNodes() {
    ...
    if (this._isActive && this._isSame(operation, filter)) {
      ...
      this._keypoints.forEach((kp, idx) => {
        ...
        applyElementStyles(kpImage, STYLES.image);
        kp.image = kpImage;
  ...
  _setOriginalSrc(src) {
    ...
    if (image.src === src) {
      ...
      applyElementStyles(this, this._fromViewer ? STYLES['active_from_viewer'] : STYLES['active_from_cropper']);
      return;
    }
    image.style.opacity = '0';
    let stop = this._handleImageLoading(src);
  ...
  deactivate({ hide = true, seamlessTransition = true } = {}) {
    ...
    if (hide && !this._hidden) {
      ...
      if (!seamlessTransition) {
        applyElementStyles(this, STYLES['inactive_rough']);
      }
...
EditorImageFader.styles = STYLES;

EditorImageFader.template = /*html*/ ``;
