...
const STYLES = {
  svg: {
    ...
    borderTopRightRadius: 'var(--border-radius-base)',
  },
  thumb: {
...
export class CropFrame extends AppComponent {
  ...
  _createBackdrop() {
    ...
    let maskRectOuter = createSvgNode('rect', { x: 0, y: 0, width: '100%', height: '100%', fill: 'white' });
    let maskRectInner = createSvgNode('rect', { x, y, width, height, fill: 'black' });
  ...
  _updateFrame() {
    ...
    for (let thumb of Object.values(this._frameThumbs)) {
      ...
      if (isCenter) {
        ...
        setSvgNodeAttrs(interactionNode, { r: Math.min(width, height) / 3, cx: center[0], cy: center[1] });
      } else {
      ...
      applyElementStyles(groupNode, disableThumb ? STYLES['thumb--hidden'] : STYLES['thumb--visible']);
    }
  ...
  _createThumbs() {
    ...
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
        ...
        applyElementStyles(groupNode, STYLES['thumb']);
        groupNode.setAttribute('with-effects', '');
        let interactionNode = createSvgNode('circle', { r: THUMB_CORNER_SIZE + THUMB_OFFSET, fill: 'transparent' });
        ...
        frameThumbs[direction] = { direction, pathNode, interactionNode, groupNode };
  ...
  _createGuides() {
    ...
    applyElementStyles(svg, STYLES['guides']);
    applyElementStyles(svg, STYLES['guides--semi-hidden']);
  ...
  _handlePointerMove_(e) {
    ...
    if (!Object.values(rect).every((number) => Number.isFinite(number) && number >= 0)) {
      console.error('CropFrame is trying to create invalid rectangle', { payload: rect });
      return;
    }
  ...
  toggleThumbs(visible) {
    Object.values(this._frameThumbs)
      ...
      .forEach((groupNode) => {
        applyElementStyles(groupNode, visible ? STYLES['thumb--visible'] : STYLES['thumb--hidden']);
      });
  }
  ...
  readyCallback() {
    ...
    this.sub('dragging', (dragging) => {
      if (this._guidesHidden) {
        applyElementStyles(this._frameGuides, STYLES['guides--hidden']);
      } else {
        applyElementStyles(this._frameGuides, STYLES[dragging ? 'guides--visible' : 'guides--semi-hidden']);
      }
    });
...
CropFrame.styles = STYLES;

CropFrame.template = /*html*/ `
  <svg css='svg' ref='svg-el' xmlns="http://www.w3.org/2000/svg">
  </svg>
`;
