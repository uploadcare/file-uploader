...
export class CropFrame extends AppComponent {
  ...
  _shouldThumbBeDisabled(direction) {
    ...
    if (
      direction === '' &&
      imageBox.height <= MIN_CROP_SIZE &&
      imageBox.width <= MIN_CROP_SIZE
    ) {
      return true
    }
  ...
  _createThumbs() {
    ...
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
        ...
        interactionNode.addEventListener(
          'pointerdown',
          this._handlePointerDown.bind(this, direction),
        )
      }
    }
  ...
  _updateCursor() {
    ...
    this.ref('svg-el').style.cursor = hoverThumb
      ? thumbCursor(hoverThumb.direction)
      : 'initial'
  }
