...
const STYLES = {
  ...
  active_from_editor: {
    transition:
      'transform var(--transition-duration-4) cubic-bezier(0.37, 0, 0.63, 1) .4s, opacity var(--transition-duration-3)',
    opacity: '1',
    pointerEvents: 'auto',
  ...
  inactive_to_editor: {
    transition:
      'transform var(--transition-duration-4) cubic-bezier(0.37, 0, 0.63, 1), opacity var(--transition-duration-3) calc(var(--transition-duration-3) + 0.05s)',
    opacity: '0',
    pointerEvents: 'none',
  },
  inactive_instant: {
...
export class EditorImageCropper extends AppComponent {
  ...
  _alignCrop() {
    ...
    if (transformation) {
      ...
      cropBox = { x: imageBox.x, y: imageBox.y, width: imageBox.width, height: imageBox.height };
    }
    /** @type {[number, number]} */
  ...
  _animateIn({ fromViewer }) {
    if (this.state.image) {
      ...
      setTimeout(() => {
        applyElementStyles(this, fromViewer ? STYLES['active_from_viewer'] : STYLES['active_from_editor']);
      });
    }
  }
  ...
  _calculateCrop() {
    ...
    if (!validateCrop(crop)) {
      console.error('Cropper is trying to create invalid crop object', { payload: crop });
      return undefined;
    }
  ...
  deactivate({ seamlessTransition = false } = {}) {
    ...
    if (seamlessTransition) {
      this._alignTransition();
      applyElementStyles(this, STYLES['inactive_to_editor']);
    } else {
      applyElementStyles(this, STYLES['inactive_instant']);
    }
  ...
  _alignTransition() {
    let dimensions = this._calculateDimensions();
    let scaleX = Math.min(this.offsetWidth, dimensions[0]) / this.state.cropBox.width;
    let scaleY = Math.min(this.offsetHeight, dimensions[1]) / this.state.cropBox.height;
...
EditorImageCropper.styles = STYLES;

EditorImageCropper.template = /*html*/ `
  <canvas css='canvas' ref='canvas-el'></canvas>
