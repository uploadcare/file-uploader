...
describe('uc-drop-area — uploader-scope-attach (solution-only composition, no provider)', () => {
  it('attaches the full uploader scope from a bare solution + config composition (no <uc-upload-ctx-provider>)', async () => {
    ...
    expect(typeof (dropArea as unknown as { api: { openSystemDialog: () => void } }).api.openSystemDialog).toBe(
      'function',
    );
    expect((dropArea as unknown as { uploadController: unknown }).uploadController).toBeTruthy();
  });
});
...
describe('uc-drop-area — enable / visibility / _shouldIgnore proxies', () => {
  ...
  it('multiple:false plus an existing file blocks further drop handling (drag-state never advances past inactive)', async () => {
    ...
    const dropAreaWithApi = dropArea as unknown as { api: { addFileFromUrl: (url: string) => void } };
    dropAreaWithApi.api.addFileFromUrl(TEST_IMAGE_URL);
    await expect
      .poll(() => (dropArea as unknown as { uploadCollection: { size: number } }).uploadCollection.size)
      .toBe(1);
