...
const request = ({ method, url, data, headers = {}, signal, onProgress }) =>
...
const createQuery = (query) =>
  Object.entries(query)
    .reduce(
      (params, [key, value]) =>
        params.concat(
          Array.isArray(value) ? value.map((value) => serializePair(`${key}[]`, value)) : serializePair(key, value)
        ),
      []
    )
    .filter((x) => !!x)
...
const getUrl = (base, path, query) =>
  [base, path, query && Object.keys(query).length > 0 ? '?' : '', query && createQuery(query)].filter(Boolean).join('');
...
function retrier(fn, options = defaultOptions) {
  ...
  function runAttempt(fn) {
    ...
    const retry = (ms) =>
      delay(ms !== null && ms !== void 0 ? ms : defaultDelayTime).then(() => {
        attempts += 1;
    ...
    fn().catch((error) => {
      ...
      if (
        ...
        ((_a = error === null || error === void 0 ? void 0 : error.response) === null || _a === void 0
          ? void 0
          : _a.statusCode) === REQUEST_WAS_THROTTLED_CODE &&
...
/** Performs file uploading request to Uploadcare Upload API. Can be canceled and has progress. */
function base(
  file,
  {
    publicKey,
    ...
    return request({
      ...
      data: buildFormData([
        [
          'file',
          file,
          (_a = fileName !== null && fileName !== void 0 ? fileName : file.name) !== null && _a !== void 0
            ? _a
            : defaultFilename,
        ],
...
const poll = ({ check, interval = DEFAULT_INTERVAL, signal }) =>
  new Promise((resolve, reject) => {
    let timeoutId;
    onCancel(signal, () => {
      ...
      fromUrlStatus(token, {
        ...
        switch (response.status) {
          ...
          case Status.Progress: {
            if (onProgress) onProgress({ value: response.done / response.total });
            return false;
          }
          case Status.Success: {
            if (onProgress) onProgress({ value: response.done / response.total });
...
const pushStrategy = ({ token, pusherKey, signal, stopRace, onProgress }) =>
  new Promise((resolve, reject) => {
...
const uploadPartWithRetry = (chunk, url, { onProgress, signal, multipartMaxAttempts }) =>
    ...
    .then(({ uuid, parts }) =>
      Promise.all([
        ...
        runWithConcurrency(
          ...
          parts.map(
            (url, index) => () =>
              uploadPartWithRetry(getChunk(file, index, size, multipartChunkSize), url, {
...
function uploadFileGroup(
  ...
  return Promise.all(
    ...
    const addDefaultEffects = (file) => {
      ...
      return Object.assign(Object.assign({}, file), { cdnUrlModifiers, cdnUrl });
    };
